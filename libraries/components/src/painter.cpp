...
void paint_visitor::paint(const area &obj)
{
	...
	sf::RectangleShape* board = new sf::RectangleShape{ {config.begin.x, config.begin.y} };
...
void paint_visitor::paint(const ship &obj)
{
	...
	for (point &p : *points_to_paint)
	{
		...
		sf::RectangleShape* cell = new sf::RectangleShape{{coord.x, coord.y}};
...
pixel_coord paint_visitor::get_point_position(const point &p) const
{
	...
	if (p == point{0, 0})
		...
		return pixel_coord{
			top_left.x + (config.get_cell_width() * p.x) + (config.padding * 2.0f * (p.x - 1.0f)),
			top_left.y + (config.get_cell_height() + p.y) + (config.padding * 2.0f * (p.y - 1.0f))
		};
}